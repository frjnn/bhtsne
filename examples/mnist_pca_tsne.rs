...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let labels: Vec<u8> = npz.by_name("labels")?.expect("missing 'labels'").into_vec()?;
...
fn plot_embedding(
    ...
    let (x_min, x_max, y_min, y_max) = embedding
        .chunks_exact(2)
        .fold(
            (f64::INFINITY, f64::NEG_INFINITY, f64::INFINITY, f64::NEG_INFINITY),
            |(xlo, xhi, ylo, yhi), c| {
                (xlo.min(c[0]), xhi.max(c[0]), ylo.min(c[1]), yhi.max(c[1]))
            },
        );
