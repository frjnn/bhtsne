...
/// Per-dimension Z-order codec, implemented only for [`Dim<2>`] and [`Dim<3>`]. `#[doc(hidden)]` and
/// sealed by the orphan rule, but `pub` so the `Dim<D>: Morton<D>` bound can sit on the public API.
#[doc(hidden)]
...
mod tests {
    ...
    fn encode_decode_roundtrips_2d() {
        ...
        for _ in 0..10_000 {
            let x = (lcg(&mut seed) & 0xffff_ffff) as u32;
            let y = (lcg(&mut seed) & 0xffff_ffff) as u32;
    ...
    fn encode_decode_roundtrips_3d() {
        ...
        let mask = (1u32 << 21) - 1;
        for _ in 0..10_000 {
            let x = (lcg(&mut seed) as u32) & mask;
            let y = (lcg(&mut seed) as u32) & mask;
            let z = (lcg(&mut seed) as u32) & mask;
